...
class ShoppingListDemo {
    ...
    try {
      ...
      if (process.stdin.isTTY) {
        ...
        while (true) {
          const ans = (
            await this.ask(
              "\nDigite o número da categoria (ou C para cancelar): "
            )
          ).trim();
          if (!ans) {
          ...
          if (Number.isNaN(idx) || idx < 1 || idx > categories.length) {
            console.log(
              "Opção inválida, digite o número correspondente à categoria."
            );
            continue;
          }
      ...
      const itemsResponse = await axios.get(
        `${this.baseUrl}/api/items?category=${encodeURIComponent(
          category
        )}&limit=5`
      );
      ...
      if (items.length > 0) {
        ...
        items.forEach((it, i) => {
          ...
          const price =
            it.averagePrice !== undefined && it.averagePrice !== null
              ? ` | Preço médio: R$ ${it.averagePrice}`
              : "";
          const desc = it.description ? ` | ${it.description}` : "";
          console.log(
            `${i + 1}. ${name} (id: ${id})${brand}${unit}${price}${desc}`
          );
        });
      }
  ...
  async searchItems() {
    ...
    try {
      const q = 'arroz';
      const url = `${this.baseUrl}/api/items/search?q=${encodeURIComponent(q)}`;

      const headers = this.token ? { Authorization: `Bearer ${this.token}` } : {};
      const response = await axios.get(url, { headers });

      // suporte a diferentes formatos: data.items ou data diretamente como array
      let items = response.data?.data?.items ?? response.data?.data ?? [];
      let count = Array.isArray(items) ? items.length : 0;

      // Fallback: se search do serviço retornar vazio, buscar items gerais e filtrar localmente
      if (count === 0) {
        const allUrl = `${this.baseUrl}/api/items?limit=1000`;
        const allResp = await axios.get(allUrl, { headers });
        ...
        if (Array.isArray(allItems) && allItems.length > 0) {
          ...
          items = allItems.filter((it) => {
            ...
            const brand = (it.brand || '').toString().toLowerCase();
            return name.includes(qLower) || desc.includes(qLower) || brand.includes(qLower);
          });
          count = items.length;
        } else {
        }
      ...
      console.log(`Resultados da busca por "${q}":`, count);

      if (Array.isArray(items) && items.length > 0) {
        console.log('Detalhes dos itens encontrados:');
        items.forEach((it, i) => {
          const name = it.name || '(sem nome)';
          const id = it.id || '-';
          const brand = it.brand ? ` | Marca: ${it.brand}` : '';
      ...
      // se o formato for inesperado, imprime sugestão de debug
      if (!Array.isArray(items)) {
        console.log('Formato inesperado de resposta. Verifique o endpoint /api/items/search no gateway/service.');
      }
  ...
  // Novo submenu exibido após login bem-sucedido com as operações de lista
  async showListMenu() {
    const readline = require("readline");
    const rl = readline.createInterface({
      input: process.stdin,
    ...
    while (!exit) {
      ...
      switch (choice) {
        case 1:
          if (!this.token) {
            console.log("Ação requer autenticação. Faça login primeiro.");
          } else {
  ...
  async showMenu() {
    ...
    while (!exit) {
      ...
      switch (choice) {
        ...
        case 4:
          {
            const ok = await this.loginUser();
            if (ok) {
              // abre o submenu de listas imediatamente após login
              await this.showListMenu();
            } else {
              console.log("Login falhou, não é possível acessar o menu de listas.");
            }
          }
          break;
        case 5:
